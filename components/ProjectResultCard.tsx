...
const ProjectResultCard = ({ projects }: ProjectResultCardProps) => {
    return (
            ...
            <div className="space-y-4 stagger-children">
                {projects.map((project, index) => (
                    <div 
                        ...
                        className="bg-slate-900/50 backdrop-blur-sm p-4 rounded-lg border border-emerald-300/10 transition-all duration-300 hover:border-emerald-300/30 hover:bg-slate-800/60"
                    >
                        <h4 className="font-semibold text-emerald-300">{project.title}</h4>
                        <p className="text-slate-400 mt-1 text-sm leading-relaxed">{project.description}</p>
                        <div className="mt-4">
                            <a
